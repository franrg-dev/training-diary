    ...
    <div style={{ padding: '0 16px 16px' }}>
        ...
        <div className="app-card" style={{ marginBottom: '16px' }}>
          ...
          {ultimosCardio.map((r, i) => {
            ...
            return (
              <div
                key={r.id}
                style={{ paddingBottom: i < ultimosCardio.length - 1 ? '10px' : 0, marginBottom: i < ultimosCardio.length - 1 ? '10px' : 0, borderBottom: i < ultimosCardio.length - 1 ? '1px solid #2e2e2e' : 'none' }}
              >
                <p style={{ margin: '0 0 4px', fontSize: '12px', color: '#6b7280' }}>{formatearFecha(r.fecha)}</p>
                <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                  {r.duracion && <span><span style={{ fontSize: '16px', fontWeight: '700', color: '#f97316' }}>{r.duracion}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '3px' }}>min</span></span>}
                  {r.ritmo    && <span><span style={{ fontSize: '16px', fontWeight: '700', color: '#f97316' }}>{r.ritmo}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '3px' }}>{ritmoLabel}</span></span>}
                  {r.volumen  && <span><span style={{ fontSize: '16px', fontWeight: '700', color: '#f97316' }}>{r.volumen}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '3px' }}>{volumenLabel}</span></span>}
                </div>
              </div>
            )
          })}
      ...
      {registros.length === 0 && (
        <div style={{ textAlign: 'center', marginTop: '8px', marginBottom: '20px' }}>
          ...
          <p style={{ margin: 0, fontSize: '14px', color: '#6b7280' }}>Los registros se añaden desde el diario</p>
        </div>
      )}
      ...
      {registros.length > 0 && (
        <div className="app-card">
          ...
          {registros.slice().reverse().map((r, i) => (
            <div
              key={r.id}
              style={{ borderBottom: i < registros.length - 1 ? '1px solid #2e2e2e' : 'none', paddingBottom: i < registros.length - 1 ? '12px' : 0, marginBottom: i < registros.length - 1 ? '12px' : 0 }}
            >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {esCardio ? (
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                      {r.duracion && <span><span style={{ fontSize: '18px', fontWeight: '700', color: '#f97316' }}>{r.duracion}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '2px' }}>min</span></span>}
                      {r.ritmo    && <span><span style={{ fontSize: '18px', fontWeight: '700', color: '#f97316' }}>{r.ritmo}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '2px' }}>{r.modo === 'veces' ? 'Rp/m' : 'Km/h'}</span></span>}
                      {r.volumen  && <span><span style={{ fontSize: '18px', fontWeight: '700', color: '#f97316' }}>{r.volumen}</span><span style={{ fontSize: '11px', color: '#6b7280', marginLeft: '2px' }}>{r.modo === 'veces' ? 'Rp' : 'Km'}</span></span>}
                    </div>
                  ) : (
                    <div>
              ...
              {r.notas && (
                <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#6b7280', fontStyle: 'italic' }}>{r.notas}</p>
              )}
              {confirmarBorrarId === r.id && (
                <div style={{ marginTop: '8px', display: 'flex', gap: '8px' }}>
                  <button onClick={() => setConfirmarBorrarId(null)} style={{ flex: 1, padding: '8px', backgroundColor: 'transparent', border: '1px solid #2e2e2e', borderRadius: '8px', color: '#a1a1a1', fontSize: '13px', cursor: 'pointer' }}>Cancelar</button>
